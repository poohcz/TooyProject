...
public protocol BroadcastMapperProtocol {
    ...
    func map(dto: BroadcastDTO) -> BroadcastModel?
    // 배열로 실제 api기능. 리스트 보여줄때.
    func map(dtoList: [BroadcastDTO]) -> [BroadcastModel]
}
...
public final class BroadcastMapper: BroadcastMapperProtocol {
    ...
    public func map(dto: BroadcastDTO) -> BroadcastModel? {
    ...
    public func map(dtoList: [BroadcastDTO]) -> [BroadcastModel] {
